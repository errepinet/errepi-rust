...
- `protos/` is a git submodule (`errepinet-sys-services-protos`) with the gRPC `.proto` defs. Compiled at build time via `tonic-prost-build` (`compile_protos` in `build.rs`), code lives in `OUT_DIR` via `tonic::include_proto!("protos")` (`src/pb.rs`). Requires `protoc` on PATH (libprotoc 3.21.12 in use). Bump pointer with `git -C protos pull && git add protos && git commit`.
